...
#define DOWNLOADSIZ (1 << 26) // Maximum download size is 64MB
...
// block for forking(void)
// block other processes so that process can fork
pid_t blocked(void) {
    if (numChildren < 8) {
        numChildren++;
        return fork();
        ...
        pid_t pid;
        while (numChildren != 0) {
            pid = waitpid(-1, &val, WNOHANG);
            if (pid == 0) {
                // pid couldn't be retrieved
                // either too many children running or none others can be forked
                if (numChildren == 8) {
                    while (waitpid(-1, &val, 0) && WIFEXITED(val)) {;}
                    // loop until the waitpid exists
                    return fork();
                }
                else break;
            } else if (pid == -1) die("Error in blocking");
            else numChildren--;
        }
        numChildren++;
        return fork();
...
static task_t *task_new(tasktype_t type)
{
    ...
    if ( !((char *) malloc(TASKBUFSIZ * sizeof(char))) ) {
        errno = ENOMEM;
        return NULL;
    }
	strcpy(t->filename, "");
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
    if (strnlen(filename, FILENAMESIZ) == FILENAMESIZ) {
        error("Filename is longer than the allowed file name size", filename);
        goto exit;
    }
...
{
    // "return" has been changed to "exit(0)" to support parallelism
	int i, ret = -1;
    ...
    if (evil_mode == 2) {
        // this attack is characterized by the request of a file while the peer is attempting to upload it
        while (write(t->peer_fd, "attack!!!", 1) > 0) {
        
        }
        message("Continually write to peer's file until there is no more room on disk");
        goto try_again;
    }
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
        ...
        if (t->total_written + TASKBUFSIZ > DOWNLOADSIZ) {
...
static void task_upload(task_t *t)
{
    ...
    // evil_mode = 1
    // when evil_mode equals 1, there is an attack. The attack is dictated by someone attempting to upload a file that is not in the current directory of the user. Inserting "../error" as the filename allows us, the attackers, to be able to disguise our file as the original file.
    
    DIR* currentdirectory = opendir("."); // get current directory
    struct dirent* newentry = readdir(currentdirectory);
    unsigned long length = strlen(newentry->d_name);
    int inDirectory = 0;
    
    if (currentdirectory == NULL) die("Directory is open", strerror(errno));
    else {
        struct stat temp;
        while (newentry != NULL) {
            if ( (length > 1 && newentry->d_name[length - 1] == '~') || !S_ISREG(temp.st_mode) || (length > 2 && newentry->d_name[length - 2] == '.' && (newentry->d_name[length - 1] == 'c'
                        || newentry->d_name[length - 1] == 'h') || stat(newentry->d_name, &temp) < 0 ) )
                continue;
            
            if (strcmp(newentry->d_name, t->filename)) {
                inDirectory = 1;
                break;
            }
        }
    ...
    if (evil_mode == 1) t->disk_fd = open("../error", O_RDONLY);
    else t->disk_fd = open(t->filename, O_RDONLY);
	if (t->disk_fd == -1) {
